...
class TestConcurrency:
    ...
    def test_concurrent_eviction(self, tmp_path):
        ...
        assert not errors
        total = cache._conn.execute("SELECT COALESCE(SUM(size), 0) FROM cache").fetchone()[0]
        assert total < 5000 * 3
...
class TestCacheIntegration:
    def test_full_cache_put_get_cycle_with_model_response(self, tmp_path):
        from litellm import ModelResponse
    ...
    def test_disk_cache_write_failure_doesnt_crash(self, tmp_path):
        from dspy.clients.cache import Cache
    ...
    def test_cache_contains_checks_disk(self, tmp_path):
        from dspy.clients.cache import Cache
