...
class SQLiteCache:
    ...
    def __init__(self, directory: str, size_limit: int | None = None):
        ...
        db_path = os.path.join(directory, "dspy_cache.db")
        ...
        self._conn.execute("CREATE INDEX IF NOT EXISTS idx_last_access ON cache(last_access)")
        self._conn.commit()
    ...
    def __getitem__(self, key: str) -> Any:
        with self._lock:
            ...
            if row is None:
                raise KeyError(key)
            self._conn.execute("UPDATE cache SET last_access = ? WHERE key = ?", (time.time(), key))
            self._conn.commit()
    ...
    def __eq__(self, other: object) -> bool:
        if isinstance(other, dict) and len(other) == 0:
            with self._lock:
                return self._conn.execute("SELECT COUNT(*) FROM cache").fetchone()[0] == 0
    ...
    def _maybe_evict(self) -> None:
        ...
        if self._size_limit is None or self._size_limit <= 0:
            return
        total_size = self._conn.execute("SELECT COALESCE(SUM(size), 0) FROM cache").fetchone()[0]
        if total_size <= self._size_limit:
            return
        overage = total_size - self._size_limit
        evicted = 0
        rows = self._conn.execute("SELECT key, size FROM cache ORDER BY last_access ASC").fetchall()
        keys_to_delete = []
        ...
        if keys_to_delete:
            ...
            self._conn.execute(f"DELETE FROM cache WHERE key IN ({placeholders})", keys_to_delete)
            self._conn.commit()
    ...
    for shard_id in range(16):
        ...
        for key, value_blob, mode, access_time, filename in rows:
            try:
                ...
                entries.append((key, blob, len(blob), access_time or time.time()))
            except (
